...
import (
	...
	"github.com/duyanghao/Registry_Notification_Server/Handler_event"
	"net/http"
	"strings"
)

var con_fig *Configuration.Config

//map handler
var DIS_RULE = map[string]func(http.ResponseWriter, *http.Request, notifications.Event, *Configuration.Config) error{
	"pull": Handler_event.ProcessPullEvent,
	"push": Handler_event.ProcessPushEvent,
}

type Dispatcher struct {
	disRule map[string]func(http.ResponseWriter, *http.Request, notifications.Event, *Configuration.Config) error
}
type Server struct {
...
func (server Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(uri_string) > 2 {
		if uri_string[1] == "search" { //for search handler
			Handler_event.ProcessSearch(w, r, con_fig)
			return
		} else if uri_string[1] == "analysis" { //for analysis handler
			Handler_event.ProcessAnalysis(w, r, con_fig)
			return
...
func main() {
	...
	fmt.Println("INFO: Starting Loading configuration!")
	_, err := Configuration.LoadConfig(configFilePath)
	if err != nil {
		fmt.Printf("Error: %s\n", err)
		return
	}
	con_fig, _ = Configuration.LoadConfig(configFilePath)
